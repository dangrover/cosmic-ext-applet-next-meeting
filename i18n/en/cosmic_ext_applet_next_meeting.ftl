...
location-section = Indicate location
...
no-calendar-service = Calendar service not found
no-calendar-service-description = This applet reads calendars from Evolution Data Server, which doesn't appear to be installed. Install a calendar app such as GNOME Calendar or Evolution, then add your calendar account.

# Scheduling Helper
scheduling-helper = Scheduling Helper
scheduling-helper-description = Compose your availability from open calendar slots and copy it as a message.
scheduling-section = Scheduling
scheduling-enable = Show scheduling helper
scheduling-window-mode = Window
scheduling-window-fixed = Fixed popup
scheduling-window-resizable = Resizable window
scheduling-window-description = A fixed popup opens as a compact dialog (floats on tiling window managers); a resizable window can be resized but may be tiled.
on = On
off = Off
your-message = Your message
scheduling-message-intro = I'm available between the following times ({ $timezone }) — let me know what works:
scheduling-options = Options
scheduling-pick-times = Pick open times
scheduling-copy = Copy
scheduling-clear = Clear
scheduling-auto-pick = Auto-pick
scheduling-empty-hint = Drag across the grid to mark when you're available.
scheduling-loading = Loading your calendar…
from-label = From
to-label = To
range-label = Range
...
calendars-all = All
calendars-summary = { $selected } of { $total }
...
keyboard-shortcut-description = You can set up a system keyboard shortcut for a Meeting action.
...
shortcut-action-open-menu = Open menu
shortcut-action-next-meeting = Open next meeting
shortcut-action-scheduling-helper = Open Scheduling Helper
keyboard-shortcut-command = cosmic-ext-applet-next-meeting --join-next
...
hide-when-no-meetings = Hide if no meetings